...
const RestaurantMenu = () => {
  ...
  const itemCards2 =  menuInfo?.cards[5]?menuInfo?.cards[5]: menuInfo?.cards[4];
  const { itemCards } =
  itemCards2.groupedCard?.cardGroupMap?.REGULAR?.cards[2]?.card
      ?.card;
   
  const catagories =
  itemCards2?.groupedCard?.cardGroupMap?.REGULAR?.cards.filter(
      (c) =>
        c.card?.card?.["@type"] ===
